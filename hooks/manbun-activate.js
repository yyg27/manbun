#!/usr/bin/env node
// manbun — Claude Code SessionStart activation hook (also Codex, Copilot,
// Grok and Cursor sessionStart)
//
// Runs on every session start:
//   1. Writes flag file at $CLAUDE_CONFIG_DIR/.manbun-active (defaults to ~/.claude; statusline reads this)
...
if (!isCodex && !isCopilot && !isCursor) try {
  ...
  const nudgeFlagPath = path.join(claudeDir, '.manbun-statusline-nudged');
  if (!hasStatusline && !fs.existsSync(nudgeFlagPath)) {
    ...
    if (isShellSafe(scriptPath)) {
      ...
      // manbun: install path has shell metacharacters — don't embed it in a
