...
const os = require('os');
const { getClaudeDir, getConfigDir } = require('./manbun-config');

const STATE_FILE = '.manbun-active';

// manbun: VS Code Copilot never sets COPILOT_PLUGIN_DATA — it only injects
// CLAUDE_PLUGIN_ROOT, pointed at an install path under .vscode/agent-plugins/
// (#528). Without this fallback isCopilot was false, so manbun assumed
...
// Live mode written by activate/mode-tracker. Absent flag = manbun off.
function readMode() {
  try {
...
// Cursor's always-on project rule (.cursor/rules/manbun.mdc) already puts the
...
// manbun: first workspace root only, a rule in a secondary folder of a
// multi-root workspace goes undetected.
function cursorRulePath() {
  const root = process.env.CURSOR_PROJECT_DIR || process.cwd();
  const rule = path.join(root, '.cursor', 'rules', 'manbun.mdc');
  return fs.existsSync(rule) ? rule : null;
}

function cursorRuleNotice(rule) {
  return 'MANBUN: the always-on Cursor rule ' + rule + ' is active in this workspace and ' +
    'already carries the manbun ruleset, so the manbun hooks injected nothing further. ' +
    'Mode switching (/manbun lite|full|ultra|off, "stop manbun") is unavailable while ' +
    'that rule exists. When the user tries to switch or turn off manbun, tell them to ' +
...
function writeHookOutput(event, mode, context = '') {
  ...
  if (isCodex) {
    const output = { systemMessage: `MANBUN:${mode.toUpperCase()}` };
    if (context) {
