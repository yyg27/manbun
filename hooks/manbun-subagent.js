#!/usr/bin/env node
// manbun — Claude Code SubagentStart hook
//
// SessionStart context is parent-thread only and never reaches subagents, so
// without this every Task-spawned agent runs manbun-unaware (issue #252).
// When manbun mode is active, inject the same ruleset into each subagent.
//
// Scoping (opt-in, issue #506): set MANBUN_SUBAGENT_MATCHER to a regex and
...
const { readMode, writeHookOutput } = require('./manbun-runtime');

const mode = readMode();

// Absent flag or off → manbun isn't active; inject nothing.
if (!mode || mode === 'off') {
  process.exit(0);
}
...
function inject() {
  try {
    writeHookOutput('SubagentStart', mode, getManbunInstructions(mode));
  } catch (e) {
...
let matcherRe = null;
try {
  if (process.env.MANBUN_SUBAGENT_MATCHER) {
    matcherRe = new RegExp(process.env.MANBUN_SUBAGENT_MATCHER, 'i');
  }
} catch (e) {
  matcherRe = null;
}
