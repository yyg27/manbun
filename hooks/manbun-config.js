...
// "stop manbun" / "normal mode" turn manbun off, but only as a standalone
...
function isDeactivationCommand(text) {
  ...
  return t === 'stop manbun' || t === 'normal mode';
}
...
function getDefaultMode() {
  ...
  // manbun: a default must be a runtime level (off/lite/full/ultra); review is
...
// Silence the pi "Manbun loaded" startup toast while keeping manbun active.
// MANBUN_QUIET_STARTUP=1 (or any truthy value; 0/false/empty mean "show it")
// takes precedence, else config.quietStartup === true. Mirrors getHideStatus.
function getQuietStartup() {
  const env = process.env.MANBUN_QUIET_STARTUP;
  if (env !== undefined) {
...
// Hide the status-bar indicator while keeping manbun active (#324).
// MANBUN_HIDE_STATUS=1 (or any truthy value; 0/false/empty mean "don't hide")
// takes precedence, else config.hideStatus === true.
function getHideStatus() {
  const env = process.env.MANBUN_HIDE_STATUS;
  if (env !== undefined) {
    const v = env.trim().toLowerCase();
    return v !== '' && v !== '0' && v !== 'false' && v !== 'no';
  }
...
function writeDefaultMode(mode) {
  // manbun: only a runtime level can be a default; review is session-only (#377).
