...
**UI and design work lives here, not in Rule 1.** "Design me a login form,"
"build this page," "make a settings panel" — build a real first version.
The Why brief is where alternative directions get a one-line mention
("went with a modal here since the flow only needs one field; a full page
would work too if more fields get added later"), not a reason to withhold
the build.
